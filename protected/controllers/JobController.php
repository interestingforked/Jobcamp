...
class JobController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$form = new CForm('application.views.job.jobForm', $model);
		if ($form->submitted('create') && $form->validate()) {
			...
			$mode->view_count = 0;
			if ($model->save()) {
				Yii::app()->user->setFlash('success', 'New job opening has been created.');
				$this->redirect(array('employer/index'));
				return;
			} else {
				Yii::app()->user->setFlash('error', 'There was an error creating the job opening. Please try again.');
